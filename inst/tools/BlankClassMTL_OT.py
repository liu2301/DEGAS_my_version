...
def CORAL_loss(source,target):
	...
	return loss
#***********************************************************************
# Calculate the L2 distance between source and target domain samples (Updated 2022/06/29)
def L2_dist(source_samples, target_samples):
     dist = tf.reshape(tf.math.reduce_sum(tf.math.square(source_samples), 1), [-1, 1])
     dist += tf.reshape(tf.math.reduce_sum(tf.math.square(target_samples), 1), [1, -1])
     dist -= 2.0 * tf.matmul(source_samples, tf.transpose(target_samples))
     return dist

# OT loss from joint deep ot from github (Updated 2022/06/29)
def OT_loss(source_samples, target_samples, trans_P):
    '''
    Compute the OT distance between source and target samples
    '''
    dist = L2_dist(source_samples, target_samples)
    loss = tf.math.reduce_sum(tf.math.multiply(trans_P, dist))
    return loss
...
# storage the transnportation plan (updated 2022/06/29)
trans_P = tf.placeholder(tf.float32, shape=[None, None]) 
#********************************************************
...
# use OT to replace MMD loss (updated 2022/07/06)
loss = 2*lossLabel2 + lambda3*lossOT + lossConstSCtoPT
